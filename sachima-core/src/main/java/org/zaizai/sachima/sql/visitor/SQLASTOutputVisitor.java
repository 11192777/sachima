...
import org.zaizai.sachima.sql.ast.statement.SQLInsertStatement.ValuesClause;
import org.zaizai.sachima.sql.ast.statement.SQLJoinTableSource.JoinType;
import org.zaizai.sachima.sql.ast.statement.SQLMergeStatement.MergeInsertClause;
import org.zaizai.sachima.sql.ast.statement.SQLMergeStatement.MergeUpdateClause;
...
import org.zaizai.sachima.sql.dialect.oracle.parser.OracleFunctionDataType;
...
import static org.zaizai.sachima.util.Utils.getBoolean;
...
public class SQLASTOutputVisitor extends SQLASTVisitorAdapter implements ParameterizedVisitor, PrintableVisitor {
    ...
    protected void printSelectList(List<SQLSelectItem> selectList) {
        ...
        for (int i = 0, lineItemCount = 0, size = selectList.size(); i < size; ++i, ++lineItemCount) {
            ...
            boolean methodOrBinary = !(selectItemExpr instanceof SQLName) && (selectItemExpr instanceof SQLMethodInvokeExpr || selectItemExpr instanceof SQLBinaryOpExpr);
    ...
    public boolean visit(SQLBinaryOpExpr x) {
        ...
        if (CollectionUtils.isNotEmpty(inputParameters) && operator == SQLBinaryOperator.Equality && x.getRight() instanceof SQLVariantRefExpr) {
    ...
    public boolean visit(SQLCaseStatement x) {
        ...
        if (!x.getElseStatements().isEmpty()) {
            println();
            print0(ucase ? "ELSE " : "else ");
...
//                    }
    ...
    public boolean visit(SQLMethodInvokeExpr x) {
        ...
        if (parameterized) {
            ...
            if (x.methodNameHashCode64() == TokenFnvConstants.TRIM && arguments.size() == 1 && arguments.get(0) instanceof SQLCharExpr && x.getTrimOption() == null && x.getFrom() == null) {
                print('?');
    ...
    protected void printMethodParameters(SQLMethodInvokeExpr x) {
        //The brackets are gone. so... return
        if (!x.getHoldEmptyArgumentBracket()) {
            return;
        }
        ...
        for (int i = 0, size = parameters.size(); i < size; ++i) {
            if (i != 0) {
                print0(x.getArgumentsSeparator());
            }
        ...
        if (using != null) {
            odpsTransformUsing = x.methodNameHashCode64() == TokenFnvConstants.TRANSFORM;
        ...
        if (x.methodNameHashCode64() == TokenFnvConstants.WEIGHT_STRING) {
    ...
    public boolean visit(SQLAggregateExpr x) {
        ...
        boolean parameterized = this.parameterized;
        if (x.methodNameHashCode64() == TokenFnvConstants.GROUP_CONCAT) {
            this.parameterized = false;
        }
        if (x.methodNameHashCode64() == TokenFnvConstants.COUNT) {
            List<SQLExpr> arguments = x.getArguments();
            if (arguments.size() == 1) {
    ...
    public boolean visit(SQLNullExpr x) {
        SQLObject parent = x.getParent();
        if (this.parameterized && (parent instanceof ValuesClause || parent instanceof SQLInListExpr || (parent instanceof SQLBinaryOpExpr && ((SQLBinaryOpExpr) parent).getOperator() == SQLBinaryOperator.Equality))) {
            print('?');
            incrementReplaceCunt();
    ...
    public boolean visit(SQLSelectQueryBlock x) {
        ...
        if (distributeBy != null && !distributeBy.isEmpty()) {
            println();
    ...
    protected void printTableElements(List<SQLTableElement> tableElementList) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (element instanceof MySqlPrimaryKey) {
                new OraclePrimaryKey(((MySqlPrimaryKey) element).getIndexDefinition()).accept(this);
            } else {
                element.accept(this);
            }
    ...
    public boolean visit(SQLSetStatement x) {
        ...
        boolean printSet = x.getAttribute("parser.set") == Boolean.TRUE || !DbType.isOracleDbType(dbType);
        if (printSet) {
            print0(ucase ? "SET " : "set ");
        }
    ...
    public boolean visit(SQLJoinTableSource x) {
        ...
        if (left instanceof SQLJoinTableSource && ((SQLJoinTableSource) left).getJoinType() == JoinType.COMMA && x.getJoinType() != JoinType.COMMA && ((SQLJoinTableSource) left).getAttribute("ads.comma_join") == null) {
            print('(');
    ...
    public boolean visit(ValuesClause x, List<Object> parameters) {
        ...
        if (inputParameters != null && !inputParameters.isEmpty() && inputParameters.get(0) instanceof Collection && ((Collection) inputParameters.get(0)).size() == x.getValues().size()) {
            incrementIndent();
    ...
    public boolean visit(SQLTruncateStatement x) {
        ...
        } else if (!x.getPartitionsForADB().isEmpty()) {
            print0(ucase ? " PARTITION " : " partition ");
    ...
    public boolean visit(SQLOver x) {
        ...
        if (windowingBetweenEnd != null || endBound != null) {
            if (windowingType != null) {
                print(' ');
                print0(ucase ? windowingType.name : windowingType.nameLower);
            }
            print0(ucase ? " BETWEEN" : " between");
            ...
            if (beginBound != null) {
                print(' ');
                print0(ucase ? beginBound.name : beginBound.nameLower);
            }
            ...
            if (endBound != null) {
                print(' ');
                print0(ucase ? endBound.name : endBound.nameLower);
            }
        } else {
            if (windowingType != null) {
                print(' ');
    ...
    public boolean visit(SQLForeignKeyImpl x) {
        ...
        if (!x.getReferencedColumns().isEmpty()) {
            print0(" (");
    ...
    public boolean visit(SQLArrayExpr x) {
        ...
        if (expr instanceof SQLIdentifierExpr && ((SQLIdentifierExpr) expr).nameHashCode64() == TokenFnvConstants.ARRAY && printNameQuote) {
    ...
    public boolean visit(SQLSubPartitionByList x) {
        ...
        if (!x.getSubPartitionTemplate().isEmpty()) {
            this.indentCount++;
            println();
    ...
    public boolean visit(SQLCreateProcedureStatement x) {
        ...
        if (block != null) {
            ...
            if (block instanceof SQLBlockStatement && !((SQLBlockStatement) block).getStatementList().isEmpty()) {
    ...
    public boolean visit(SQLCurrentTimeExpr x) {
        ...
        print(ucase ? type.name : type.nameLower);
        return false;
    }
