...
import org.zaizai.sachima.sql.dialect.oracle.ast.stmt.OracleAlterTableSplitPartition.NestedTablePartitionSpec;
import org.zaizai.sachima.sql.dialect.oracle.ast.stmt.OracleAlterTableSplitPartition.TableSpaceItem;
import org.zaizai.sachima.sql.dialect.oracle.ast.stmt.OracleAlterTableSplitPartition.UpdateIndexesClause;
...
import org.zaizai.sachima.sql.dialect.oracle.ast.stmt.OracleSelectRestriction.CheckOption;
import org.zaizai.sachima.sql.dialect.oracle.ast.stmt.OracleSelectRestriction.ReadOnly;
...
public class OracleOutputVisitor extends SQLASTOutputVisitor implements OracleASTVisitor {
    ...
    public boolean visit(OracleExceptionStatement.Item x) {
        ...
        if (x.getStatements().size() > 1) {
            ...
            if (x.getStatements().size() == 1 && x.getStatements().get(0) instanceof SQLIfStatement) {
                println();
            } else {
                print(' ');
            }
    ...
    public boolean visit(OracleCreateIndexStatement x) {
        ...
        if (!x.getLocalPartitions().isEmpty()) {
            println();
...
//    }
    ...
    public boolean visit(OracleCreateTableStatement.Organization x) {
        ...
        if ("EXTERNAL".equalsIgnoreCase(type)) {
            ...
            if (!x.getExternalDirectoryLocation().isEmpty()) {
                println();
    ...
    public boolean visit(OracleCreatePackageStatement x) {
        ...
        if (x.isBody() || !statements.isEmpty()) {
            println();
    ...
    public boolean visit(SQLColumnDefinition x) {
        ...
        if (x.isAutoIncrement()) {
            print0("GENERATED BY DEFAULT AS IDENTITY");
        }
