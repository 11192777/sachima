...
import org.zaizai.sachima.sql.dialect.mysql.ast.expr.MySqlExpr;
import org.zaizai.sachima.sql.dialect.mysql.visitor.MySqlASTVisitorAdapter;
import org.zaizai.sachima.sql.dialect.oracle.ast.expr.OracleExpr;
import org.zaizai.sachima.sql.dialect.oracle.visitor.OracleASTVisitorAdapter;
...
public class SchemaStatVisitor extends SQLASTVisitorAdapter {
    ...
    protected final List<SQLMethodInvokeExpr> functions = new ArrayList<>(2);
...
//        }
    ...
    private static boolean isParam(SQLIdentifierExpr x) {
        if (x.getResolvedParameter() != null || x.getResolvedDeclareItem() != null) {
            return true;
        }
        return false;
    }
    ...
    public boolean visit(SQLIdentifierExpr x) {
        ...
        } else if (tableSource instanceof SQLWithSubqueryClause.Entry || tableSource instanceof SQLSubqueryTableSource || tableSource instanceof SQLLateralViewTableSource) {
            return false;
        } else {
            boolean skip = false;
            for (SQLObject parent = x.getParent(); parent != null; parent = parent.getParent()) {
